...
const VideoGenerationView: React.FC<VideoGenerationViewProps> = ({ preset, clearPreset }) => {
  ...
  const allStates = {
    ...
    referenceImage, previewUrl, resolution, aspectRatio, voiceoverLanguage
  };
  ...
  const handleGenerate = useCallback(async () => {
      ...
      try {
          ...
          const { videoUrl: streamUrl, thumbnailUrl: newThumbnailUrl, videoBlobPromise } = await generateVideo(fullPrompt, model, aspectRatio, resolution, dynamicNegativePrompt, image, savedAuthToken);

          if (streamUrl && videoBlobPromise) { // VEO 3.0 path (streaming)
              console.log('✅ Video stream URL received:', streamUrl);
              setVideoUrl(streamUrl);
              setVideoFilename(`monoklix-video-${Date.now()}.mp4`);
              setThumbnailUrl(newThumbnailUrl);

              // Handle background saving for history and logging
              videoBlobPromise.then(file => {
                  addLogEntry({ model, prompt: fullPrompt, output: '1 video generated successfully (streamed).', tokenCount: 0, status: 'Success', mediaOutput: file });
                  triggerUserWebhook({ type: 'video', prompt: fullPrompt, result: file });
                  addHistoryItem({ type: 'Video', prompt: `Video: ${fullPrompt}`, result: file });
              }).catch(err => {
                  console.error("Error saving streamed video to history:", err);
                  setError("Video played successfully, but failed to save to history.");
              });

          } else {
              throw new Error('generateVideo did not return a stream URL.');
          }
  ...
  }, [prompt, style, lighting, camera, composition, lensType, filmSim, effect, dialogue, dialogueAudio, isVeo3, referenceImage, model, aspectRatio, resolution, negativePrompt, voiceoverLanguage]);
    ...
    <>
        ...
        <div>
            ...
            <div className="space-y-4">
                ...
                <p className="text-xs text-neutral-500 dark:text-neutral-400 mt-2 p-2 bg-neutral-100 dark:bg-neutral-800/50 rounded-md" dangerouslySetInnerHTML={{ __html: 'Voiceover is only supported by <strong>Veo 3.0 models</strong> and works best with English.' }}/>
            </div>
        </div>
