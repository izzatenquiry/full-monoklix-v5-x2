        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {Array.from({ length: 4 }).map((_, i) => (
                <div key={i} className="bg-neutral-100 dark:bg-neutral-800/50 p-3 rounded-lg flex flex-col gap-3">
                    ...
                    <a
                        href={generatedImages[i] ? `data:image/png;base64,${generatedImages[i]}` : undefined}
                        download={generatedImages[i] ? `monoklix-scene-${i + 1}.png` : undefined}
                        className={`w-full text-sm bg-green-600 text-white font-semibold py-2 px-3 rounded-md hover:bg-green-700 transition-colors flex items-center justify-center gap-2 ${!generatedImages[i] ? 'opacity-50 cursor-not-allowed pointer-events-none' : ''}`}
                        onClick={(e) => { if (!generatedImages[i]) e.preventDefault(); }}
                        aria-disabled={!generatedImages[i]}
                        role="button"
                    >
