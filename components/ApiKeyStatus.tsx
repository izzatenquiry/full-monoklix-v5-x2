...
interface ApiKeyStatusProps {
    ...
    veoTokenCreatedAt: string | null;
    currentUser: User;
}

const ApiKeyStatus: React.FC<ApiKeyStatusProps> = ({ activeApiKey, veoTokenCreatedAt, currentUser }) => {
    ...
    const handleHealthCheck = async () => {
        ...
        setResults(null);
        try {
            const veoToken = sessionStorage.getItem('veoAuthToken');
            const checkResults = await runApiHealthCheck({
                textKey: activeApiKey || undefined,
                veo3AuthToken: veoToken || undefined,
            });
                                ...
                                return (
                                    <div key={index} className="p-2 bg-neutral-50 dark:bg-neutral-800 rounded-md">
                                        <div className="flex items-center justify-between gap-2">
                                         ...
                                         {result.status !== 'operational' && <p className="text-xs text-red-500 mt-1">{result.message}</p>}
                                    </div>
                                )
                            })}
                        </div>
