...
import { type View, type User, type BatchProcessorPreset, type Language, type ErrorModalContent } from './types';
...
import { MenuIcon, LogoIcon, XIcon, SunIcon, MoonIcon, CheckCircleIcon, AlertTriangleIcon, PartyPopperIcon } from './components/Icons';
import { signOutUser, logActivity, getVeoAuthToken, getTrialMasterApiKey, getAvailableApiKeys, claimApiKey } from './services/userService';
...
import { triggerUserWebhook, triggerErrorWebhook } from './services/webhookService';
...
const App: React.FC = () => {
  ...
  const [errorModalContent, setErrorModalContent] = useState<ErrorModalContent | null>(null);
  ...
  useEffect(() => {
    const fetchAndSetToken = async () => {
      ...
      if (tokenData) {
        ...
        setVeoTokenCreatedAt(tokenData.createdAt);
  ...
  useEffect(() => {
    ...
    const showErrorModal = (content: ErrorModalContent) => {
  ...
  const handleLoginSuccess = async (user: User) => {
    ...
    if (user.status === 'trial') {
        ...
        if (trialKey) {
            ...
            eventBus.dispatch('showError', 'Master API key not configured. AI features are disabled.');
        }
    } else if (!user.apiKey) {
  ...
  const handleAiSupportSend = useCallback(async (prompt: string) => {
    ...
    try {
        ...
        // Error is handled globally by handleApiError, which shows a modal.
  ...
  const handleReportError = async () => {
      if (errorModalContent?.rawError) {
          await triggerErrorWebhook(errorModalContent.rawError);
      }
  };
  ...
  // Allow admins and lifetime users full access
  if (currentUser.status === 'admin' || currentUser.status === 'lifetime') {
    isBlocked = false;
  } 
        ...
        <header className="flex items-center justify-between p-2 border-b border-neutral-200 dark:border-neutral-800 bg-white dark:bg-neutral-950 sticky top-0 z-10">
          ...
          <div className="flex items-center gap-2 pr-2">
              ...
              <ApiKeyStatus 
                ...
                veoTokenCreatedAt={veoTokenCreatedAt} 
                currentUser={currentUser}
              />
          </div>
        </header>
