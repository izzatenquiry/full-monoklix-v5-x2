...
export const triggerErrorWebhook = async (error: unknown) => {
    ...
    // Create a cleaner, more structured error object for the webhook.
    ...
    // Try to parse out HTTP status and JSON body from common error message formats.
    try {
        // Example: "Veo3 API failed (400): {...}"
        const statusMatch = errorMessage.match(/\((\d{3})\)/);
        if (statusMatch && statusMatch[1]) {
            structuredError.status = parseInt(statusMatch[1], 10);
        }

        const jsonMatch = errorMessage.match(/: (\{.*\})/);
        if (jsonMatch && jsonMatch[1]) {
            structuredError.body = JSON.parse(jsonMatch[1]);
            // Clean up the main message to avoid redundancy.
            structuredError.message = errorMessage.substring(0, jsonMatch.index);
        }
    } catch (e) {
        // If parsing fails, we still have the raw message which is valuable.
        console.warn('Could not parse structured data from error message string.');
    }

    const payload: ErrorWebhookPayload = {
        errorMessage: structuredError.message,
        errorObject: JSON.stringify(structuredError, null, 2),
        timestamp: Date.now(),
    ...
    try {
        ...
        // Fail silently. We don't want the error webhook to cause another error.
